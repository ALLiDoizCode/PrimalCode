...
const WEATHER_TRANSITIONS: Record<WeatherCondition, WeatherCondition[]> = {
  [WeatherCondition.CLEAR]: [WeatherCondition.CLOUDY, WeatherCondition.FOG],
  [WeatherCondition.CLOUDY]: [WeatherCondition.CLEAR, WeatherCondition.RAIN, WeatherCondition.STORM],
  [WeatherCondition.RAIN]: [WeatherCondition.CLOUDY, WeatherCondition.STORM, WeatherCondition.CLEAR],
  [WeatherCondition.STORM]: [WeatherCondition.RAIN, WeatherCondition.CLOUDY],
  [WeatherCondition.FOG]: [WeatherCondition.CLEAR, WeatherCondition.CLOUDY],
  [WeatherCondition.SNOW]: [WeatherCondition.CLOUDY, WeatherCondition.CLEAR]
};

const RESOURCE_GENERATION_RATES: Record<ResourceType, number> = {
  [ResourceType.FOOD]: 0.1,
  [ResourceType.WATER]: 0.05,
  [ResourceType.SHELTER]: 0.01,
  [ResourceType.HUNTING_GROUND]: 0.03,
  [ResourceType.TERRITORY]: 0.02
};
...
export class MockEnvironmentState {
  ...
  private structureIdCounter: number = 1;
  ...
  private generateStructures(): Structure[] {
    const structureTypes = Object.values(StructureType);
  ...
  private generateWeatherForecast(currentCondition: WeatherCondition): WeatherCondition[] {
    const forecast: WeatherCondition[] = [currentCondition];
  ...
  private generateEcosystemBalance(): EcosystemBalance {
  ...
  private generateRouteInfo(routeId: string, routeName: string): RouteInfo {
  ...
  private applyEnvironmentChange(environment: Environment, change: EnvironmentChange): void {
    ...
    switch (change.type) {
      case 'resource_depletion':
        environment.resources.forEach(resource => {
          const distance = Math.sqrt(
            Math.pow(resource.position.x - affectedArea.x, 2) +
      ...
      case 'structure_damage':
        environment.structures.forEach(structure => {
          const distance = Math.sqrt(
            Math.pow(structure.position.x - affectedArea.x, 2) +
