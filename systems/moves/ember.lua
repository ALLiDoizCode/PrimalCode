...
Handlers.add("ComponentResponse", Handlers.utils.hasTag("Component", "Moves"), function(msg)
  ...
  if moveName ~= "Ember" then return end
  ...
  local dmg = math.max(1, atk - def + 2)
  local burn = math.random() < 0.1

  ao.send({
    Target = replyTo,
    Tags = { Component = "MoveResult", Move = "Ember" },
    Data = json.encode({ damage = dmg, status = burn and "Burn" or nil, message = "The flames lick the foe!" })
  })
end)